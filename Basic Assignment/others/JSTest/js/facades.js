...
var module = (function() {
    ...
    var _private = {
        ...
        validateForm: function(inputs){
            ...
            } else if(inputs.args.email.trim().length == 0 || !_private.validateEmail(inputs.args.email.trim())){
        ...
        getDetails: function(args){
            ...
            for( var i = 0, len = customerArray.length; i < len; i++ ) {
                if( customerArray[i].uniqueId == args.args.uniqueId ) {
                    _private.renderDetails(customerArray[i]);
                    break;
                }
            }
        },
    ...
    return {
        facade : function( args ) {
            if (args.functionCall == 'validation' ) {
                _private.validateForm(args);
            } else if (args.functionCall == 'render' ) {
                _private.renderData();
            } else if (args.functionCall == 'search' ) {
                _private.searchData(args);
            } else if (args.functionCall == 'details' ) {
                _private.getDetails(args);
            } else if (args.functionCall == 'clear' ) {
                _private.clearData();
            } else if (args.functionCall == 'previewImg' ) {
                _private.previewImage(args);
            }
        }
    };
}());

module.facade({functionCall: "render", args:{input:false}});
function addNewCustomer() {
    var name = document.getElementById("name").value;
    var email = document.getElementById("email").value;
    var image = document.getElementById("profileSource").value;
    module.facade({functionCall: "validation", args:{name, email, image}});
}
...
function getSearchText(keyword) {
    module.facade({functionCall: "search", args:{keyword}});
}

function getCustomerDetails(uniqueId){
    module.facade({functionCall: "details", args:{uniqueId}});
}

function clearAllData() {
    if(confirm('Sure to delete all data ?')) {
        module.facade({functionCall: "clear", args:{}});
    }
}
...
function readURL(input) {
    module.facade({functionCall: "previewImg", args:{input}});
}
